...
def create_pool_of_requests():
    ...
    file = codecs.open(file_name, 'r', encoding='windows-1251', errors='ignore')
    for line in file:
...
def create_dataset():
    ...
    file_in_writer.writerow(["class_decision", "minor", "common", "help", "response"])
    for line in file_out:
        ...
        file_in_writer.writerow([str(classes[decision_model.classify(
            RequestItem(words.split(), '').features(all_words))]), str(
            get_vector.similarity(get_vector.most_similar(positive=[vector], topn=1)[0][0], "майнер_NOUN")),
                                 str(get_vector.similarity(get_vector.most_similar(positive=[vector], topn=1)[0][0],
                                                           "расписание_NOUN")), str(
