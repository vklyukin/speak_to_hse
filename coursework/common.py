...
def get_info_common(text, user_id):
    try:
        ...
        url = "http://ruz.hse.ru/ruzservice.svc/personlessons?fromdate={0}&todate={0}&receivertype=0&email={1}".format(
            str(_date), user_id)
        try:
            response = urllib.request.urlopen(url).read()
        except urllib.error.HTTPError:
            return 'Неверный запрос. Проверьте настройки почты или произнесите дату чётче.'
        except urllib.error.URLError:
            return 'Сервер недоступен в данный момент'
        else:
            output = response.decode('utf-8')
            jsonstring = json.loads(output)
            if len(jsonstring) > 0:
                answer = "<b>" + datetime.strptime(str(_date), "%Y.%m.%d").strftime("%d.%m.%Y") + "</b>, " + \
                         jsonstring[0]["dayOfWeekString"] + "\n<i>"+jsonstring[0]["building"]+"</i>\n"
                for item in jsonstring:
                    answer = answer + item['discipline'] + ', <b>' + item['auditorium'] + 'каб.</b>' + '\n<b>' + item[
                        'beginLesson'] + '-' + item['endLesson'] + '</b>\n<i>' + item['lecturer'] + '</i>\n\n'
                return answer
